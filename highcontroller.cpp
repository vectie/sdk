...
namespace legged
{
  ...
  bool HighController::init(ControlMode mode)
  {
    ...
    ddswrapper.publishModeData(cmode);
     ...
     // }
      // ddswrapper.subscribeHandReply([](const  PackageTransfer::TransferReply& ddsdata){
  ...
  void HighController::set_robotstatusdata( std::array<MotorState,24>   data,NingImuData imudata,joydata joy_data)
  {
   ...
   const std::array<MotorState,24>  HighController::get_joint_state()
  {
    ...
    const std::shared_ptr<const std::array<MotorState,24>> ms =motor_state_buffer_.GetData();
    if(ms)
    {
      for(int i=0;i<24;i++)
      {
